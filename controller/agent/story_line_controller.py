...
os.environ["LANGCHAIN_PROJECT"] = "StoryLine" + datetime.now().strftime("%d/%m/%Y %H:%M:%S")
...
class Country(BaseModel):
    ...
    country_introduction: str = Field(description="环境描写，科技水平，信仰情况等关键信息介绍", alias="关键信息")
...
class StoryLineDetail(BaseModel):
    detail_effect: str = Field(alias="本段故事作用", description="本段故事作用,描述本段故事在整体结构中发挥的作用")
    key_plot: str = Field(alias="关键情节",
                          description="关键情节,按时序描述本段故事中的关键情节，以及情节中的关键细节")
    key_role: str = Field(alias="涉及关键人物", description="涉及关键人物,给出本段故事中涉及的关键人物名")
...
class StoryLine(BaseModel):
    plot_struct_type: str = Field(alias="情节结构类型",
                                  description="情节结构类型,基于常见的故事、小说、剧作创作方法，输出你将要使用的剧情结构类型名称")
    polt_struct_character: str = Field(alias="情节结构特点",
                                       description="情节结构特点,阐述{情节结构类型}的剧情结构手法、特点")
    detailed_story_line: list[StoryLineDetail] = Field(alias="故事线详细创作",
                                                       description="故事线详细创作的顺序列表")
...
async def generate_storyline(state: StoryState):
    ...
    prompt = storyline_prompt.format(
        background=background, idea=idea, model_format_instructions=pydantic_parse.get_format_instructions())
...
async def generate_finish(state: StoryState):
    ...
    print("\n--generate_finish--\n")
    return {
        "current_detail_index": state.get("current_detail_index") - 1
    }
...
def get_graph():
    ...
    workflow.add_conditional_edges(
        ...
        generate_router,
        {"generate_finish": "generate_finish", "generate_storyline_detail": "generate_storyline_detail"})
    return workflow
